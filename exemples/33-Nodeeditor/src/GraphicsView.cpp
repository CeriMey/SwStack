...
void GraphicsView::setScene(BasicGraphicsScene *scene)
{
    ...
    {
        // setup actions
        removeAction(_clearSelectionAction);
    ...
    {
        removeAction(_deleteSelectionAction);
    ...
    {
        removeAction(_duplicateSelectionAction);
    ...
    {
        removeAction(_copySelectionAction);
    ...
    {
        removeAction(_cutSelectionAction);
        delete _cutSelectionAction;
        _cutSelectionAction = new QAction(QStringLiteral("Copy Selection"), this);
        ...
        connect(_cutSelectionAction,
                &QAction::triggered,
                this,
                &GraphicsView::onCutSelectedObjects);

        addAction(_cutSelectionAction);
    }

    {
        removeAction(_redoAction);
        delete _pasteAction;
...
void GraphicsView::onCutSelectedObjects()
{
    nodeScene()->undoStack().push(new CopyCommand(nodeScene()));
    nodeScene()->undoStack().push(new DeleteCommand(nodeScene()));
}
...
void GraphicsView::drawBackground(QPainter *painter, const QRectF &r)
{
    ...
    // Convertir le rectangle du viewport en coordonnées de la scène
    QRect viewportRect = viewport()->rect(); // Rectangle en pixels dans le viewport
    QRectF sceneRect = mapToScene(viewportRect).boundingRect(); // Convertir en scène

    backgroundPath.addRoundedRect(sceneRect, 15, 15); // Rayon des coins

    auto const &_flowViewStyle = StyleCollection::flowViewStyle();
    painter->fillPath(backgroundPath, _flowViewStyle.BackgroundColor); // Couleur de fond

    // Clip pour dessiner uniquement à l'intérieur des coins arrondis
    painter->setClipPath(backgroundPath);

    // Fonction pour dessiner la grille
    auto drawGrid = [&](double gridStep) {
        QRectF sceneRect = mapToScene(viewport()->rect()).boundingRect(); // Coordonnées de la scène
        double left = std::floor(sceneRect.left() / gridStep) * gridStep;
        double right = std::ceil(sceneRect.right() / gridStep) * gridStep;
        double bottom = std::floor(sceneRect.top() / gridStep) * gridStep;
        double top = std::ceil(sceneRect.bottom() / gridStep) * gridStep;

        // Lignes verticales
        for (double x = left; x <= right; x += gridStep) {
            QLineF line(x, bottom, x, top);
            painter->drawLine(line);
        }

        // Lignes horizontales
        for (double y = bottom; y <= top; y += gridStep) {
