...
void NodeGraphicsObject::embedQWidget()
{
    ...
    if (auto w = _graphModel.nodeData(_nodeId, NodeRole::Widget).value<QWidget *>()) {
        ...
        w->setStyleSheet(StyleCollection::widgetNodeStyle());
                               ...
                               QWidget *widget)
{
    ...
    // Masque l'embedded widget si on est trop dézoomé
    if (_proxyWidget)
    {
        if (lod < 0.05)
            _proxyWidget->setVisible(false);
        else
            _proxyWidget->setVisible(true);
    }

    // ... Votre code habituel de dessin du nœud ...
