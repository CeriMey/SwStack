...
namespace {
inline void postPortDataUpdate_(QtNodes::DataFlowGraphModel* model,
                                QtNodes::ConnectionId const& connectionId,
                                QVariant portData)
{
    SwCoreApplication* app = SwCoreApplication::instance(false);
    if (!app) {
        model->setPortData(connectionId.inNodeId,
                           QtNodes::PortType::In,
    ...
    QPointer<QtNodes::DataFlowGraphModel> safe(model);
    app->postEvent([safe, connectionId, portData]() {
        if (!safe) {
            return;
        }
        safe->setPortData(connectionId.inNodeId,
                          QtNodes::PortType::In,
                          connectionId.inPortIndex,
                          portData,
                          QtNodes::PortRole::Data);
...
DataFlowGraphModel::DataFlowGraphModel(const QString &context, std::shared_ptr<NodeDelegateModelRegistry> registry)
    : _registry(std::move(registry))
    , _nextNodeId{0}
    ,_nodesContext(context)
{
    qRegisterMetaType<NodeId>("NodeId");
    qRegisterMetaType<PortType>("PortType");
    qRegisterMetaType<PortIndex>("PortIndex");
    qRegisterMetaType<PortRole>("PortRole");
...
{
    std::unique_ptr<NodeDelegateModel> model = _registry->create(nodeType, _nodesContext);
...
bool DataFlowGraphModel::connectionPossible(ConnectionId const connectionId) const
{
    ...
    return getDataType(PortType::Out).id.remove("_debug") == getDataType(PortType::In).id.remove("_debug")
...
void DataFlowGraphModel::addConnection(ConnectionId const connectionId)
{
    ...
    QVariant const isDataPropagationEnabled = portData(connectionId.outNodeId,
                                                  PortType::Out,
                                                  connectionId.outPortIndex,
                                                  PortRole::DataPropagation);
    if(isDataPropagationEnabled.toBool()){
        QVariant const portDataToPropagate = portData(connectionId.outNodeId,
                                                      PortType::Out,
                                                      connectionId.outPortIndex,
                                                      PortRole::Data);

        postPortDataUpdate_(this, connectionId, portDataToPropagate);
    }
}
                                      ...
                                      PortRole role) const
{
    ...
    case PortRole::DataPropagation:
        result = model->dataPropagationOnConnection(portIndex);
        break;
    }
...
void DataFlowGraphModel::loadNode(QJsonObject const &nodeJson)
{
    ...
    std::unique_ptr<NodeDelegateModel> model = _registry->create(delegateModelName, _nodesContext);
...
void DataFlowGraphModel::onOutPortDataUpdated(NodeId const nodeId, PortIndex const portIndex)
{
    ...
    DebugFlowManager *_breakPoint = nullptr;
    if(_debugMode){
        foreach (DebugFlowManager *_debug, _debugFlowManagerList) {
            if(_debug->isOngoing(nodeId)){
                _breakPoint = _debug;
                break;
            }
        }

        //recuperer l'id de la connection entre les deux nodes
        if(!_breakPoint){
            _breakPoint = new DebugFlowManager(this);
            _breakPoint->start(nodeId);
            _debugFlowManagerList.append(_breakPoint);

            connect(_breakPoint, &DebugFlowManager::nodeWaiting, this, &DataFlowGraphModel::nodeWaiting);
            connect(_breakPoint, &DebugFlowManager::nodeResumed, this, &DataFlowGraphModel::nodeResumed);
        }
        _breakPoint->wait(nodeId);
    }

    for (auto const &cn : connected) {
        // setPortData(cn.inNodeId, PortType::In, cn.inPortIndex, portDataToPropagate, PortRole::Data);
        if(_debugMode) _breakPoint->start(cn.inNodeId);
        postPortDataUpdate_(this, cn, portDataToPropagate);
    }
}

void DataFlowGraphModel::resumeDebug(NodeId const nodeId)
{
    DebugFlowManager *_breakPoint = nullptr;
    foreach (DebugFlowManager *_debug, _debugFlowManagerList) {
        if(_debug->isOngoing(nodeId)){
    ...
    if(_breakPoint) _breakPoint->resume(nodeId);
}
...
bool DataFlowGraphModel::debugMode() const
{
    return _debugMode;
}

void DataFlowGraphModel::setDebugMode(bool newDebugMode)
{
    _debugMode = newDebugMode;
}

void DataFlowGraphModel::setRegistry(const std::shared_ptr<NodeDelegateModelRegistry> &newRegistry)
{
