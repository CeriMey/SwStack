...
#ifndef NDEBUG
#define SW_NODES_ASSERT(expr) \
    do { \
        if (!(expr)) { \
            swError() << "Assertion failed: " << #expr; \
            std::abort(); \
        } \
    } while (false)
#else
#define SW_NODES_ASSERT(expr) do { (void)sizeof(expr); } while (false)
#endif
...
{
    SW_NODES_ASSERT(portType != PortType::None);
...
void ConnectionGraphicsObject::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (selectedAction == deleteAction) {
        // Logique pour supprimer la connexion
//        nodeScene()->deleteConnection(_connectionId);
    } else if (selectedAction == colorAction) {
        // Logique pour changer la couleur de la connexion
//        QColor newColor = QColorDialog::getColor(Qt::yellow, nullptr, "Choose Connection Color");
//        if (newColor.isValid()) {
////            StyleCollection::connectionStyle().setConnectionColor(newColor);
//            update();
//        }
    }
...
std::pair<QPointF, QPointF> ConnectionGraphicsObject::pointsC1C2Vertical() const
{
    ...
    double verticalOffset = std::min(defaultOffset, std::abs(yDistance));
    ...
    if (yDistance <= 0) {
        ...
        horizontalOffset = std::min(defaultOffset, std::abs(xDistance)) * vector;
