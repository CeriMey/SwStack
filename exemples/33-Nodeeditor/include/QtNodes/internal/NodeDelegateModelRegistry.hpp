...
public:
    ...
    using RegistryItemCreator = std::function<RegistryItemPtr(const QString &context)>;
...
public:
    ...
    {
        RegistryItemCreator creator = [](const QString &context) { return std::make_unique<ModelType>(context); };
        registerModel<ModelType>(std::move(creator), category);
    }

    void mergeModelCreators(NodeDelegateModelRegistry::RegisteredModelCreatorsMap &target,
                            const NodeDelegateModelRegistry::RegisteredModelCreatorsMap &source) {
        for (auto &entry : source) {
            // Choisissez votre stratégie de fusion ici. Exemple : écraser les entrées existantes
            target[entry.first] = entry.second;
        }
    }

    void mergeModelCategories(NodeDelegateModelRegistry::RegisteredModelsCategoryMap &target,
                              const NodeDelegateModelRegistry::RegisteredModelsCategoryMap &source) {
        for (auto &entry : source) {
            // Fusion simple, les conflits sont gérés comme pour les créateurs de modèles
            target[entry.first] = entry.second;
        }
    }

    void mergeCategories(NodeDelegateModelRegistry::CategoriesSet &target,
                         const NodeDelegateModelRegistry::CategoriesSet &source) {
        target.insert(source.begin(), source.end());
    }

    void mergeWith(const NodeDelegateModelRegistry &other) {
        mergeModelCreators(_registeredItemCreators, other._registeredItemCreators);
        mergeModelCategories(_registeredModelsCategory, other._registeredModelsCategory);
        mergeCategories(_categories, other._categories);
    }
                        ...
                        TypeConverter          typeConverter)
  {
    ...
    std::unique_ptr<NodeDelegateModel> create(QString const &modelName, const QString &context);
