...
#include "ui/dialogs/SwCreatorStringListDialog.h"
#include "ui/dialogs/SwCreatorTextEditDialog.h"
...
void SwCreatorPropertyInspector::updateLayout_() {
    ...
    if (m_header) {
        m_header->move(r.x, r.y);
        m_header->resize(r.width, headerH);
    }

    if (m_title && m_header) {
        const SwRect hr = m_header->getRect();
        m_title->move(hr.x + pad, hr.y + (headerH - 28) / 2);
        m_title->resize(std::max(0, hr.width - 3 * pad - 70), 28);
    }
    if (m_add && m_header) {
        const SwRect hr = m_header->getRect();
        m_add->move(hr.x + hr.width - pad - 34 * 2 - 6, hr.y + (headerH - 34) / 2);
    }
    if (m_remove && m_header) {
        const SwRect hr = m_header->getRect();
        m_remove->move(hr.x + hr.width - pad - 34, hr.y + (headerH - 34) / 2);
    }

    if (m_tree) {
        const int y = r.y + headerH;
        m_tree->move(r.x, y);
...
void SwCreatorPropertyInspector::rebuild_() {
    ...
    {
        const SwRect gr = m_target->getRect();
...
void SwCreatorPropertyInspector::setEditorsForRow_(const SwModelIndex& nameIndex,
    ...
    if (propName == "StyleSheet") {
        ...
        const auto updateLayout = [cell, preview, open]() {
            ...
            open->move(r.x + r.width - buttonW, r.y);
        ...
        SwObject::connect(open, &SwToolButton::clicked, this, [this, open](bool) {
            ...
            openStyleSheetEditor_(open->getRect());
        });
