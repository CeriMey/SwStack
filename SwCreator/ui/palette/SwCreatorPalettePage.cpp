...
SwCreatorPalettePage::SwCreatorPalettePage(SwWidget* parent)
    : SwWidget(parent) {
    setStyleSheet("SwCreatorPalettePage { background-color: rgb(255, 255, 255); border-width: 0px; }");
}
...
SwRect SwCreatorPalettePage::sizeHint() const {
    SwRect hint = getRect();
    ...
    hint.height = std::max(0, contentH);
    return hint;
}

SwRect SwCreatorPalettePage::minimumSizeHint() const {
    return sizeHint();
}
...
void SwCreatorPalettePage::updateLayout_() {
    ...
    const int targetW = parentWidget ? std::max(0, parentWidget->getRect().width) : r.width;
