...
The v1 implementation starts with AV1 and targets sub-100 ms glass-to-glass latency on real
networks. The protocol prefers a recent decodable frame over a perfect late frame.

## 2) Perimetre v1

Included:
- fixed binary datagram header (`SWVT`, version, message type, stream/track ids),
- AV1 frame fragmentation under a conservative UDP MTU,
...
```text
AV1 encoder -> SwVtpAv1Packetizer -> UDP datagrams
```
...
AV1 IVF frames as SwVTP UDP datagrams. The client prints completed frames, datagram count, video
bytes, throughput, clock sync uncertainty, transfer latency, capture latency, duplicates, stale
fragments, and dropped frames.
...
latency, capture latency, duplicates, stale fragments, and dropped frames. The bitrate shown by
this example is the measured receive bitrate; full encoder-side ABR negotiation still requires
wiring receiver feedback to a real AV1 encoder target bitrate.
...
- Wire receiver feedback messages to `SwVtpAdaptiveBitrateController`.
- Add light FEC for important/keyframe fragments.
- Add KLV metadata track packetizer/reassembler.
